...
#define TSMessageViewiOS10StylePadding 4.0
...
canBeDismissedByUser:(BOOL)dismissingEnabled
{
    ...
    if ((self = [self init]))
    {
        ...
        else
        {
            ...
            self.backgroundBlurView.blurTintColor = backgroundColor;
        ...
        CGFloat textSpaceLeftPadding = [self iOS10StyleEnabled] ? padding / 2 : 2 * padding;
        self.textSpaceLeft = textSpaceLeftPadding;
        if (image) self.textSpaceLeft += image.size.width + textSpaceLeftPadding;
        ...
        if ([subtitle length])
        {
            ...
            self.backgroundBlurView.titleView.hidden = YES;
        }
        ...
        if (image)
        {
            ...
            self.iconImageView.frame = [self iOS10StyleEnabled] ? CGRectMake(padding, TSMessageViewiOS10StylePadding, 25, 25) : CGRectMake(padding * 2,padding,image.size.width,image.size.height);
        ...
        if ([buttonTitle length])
        {
            ...
            self.textSpaceRight = textSpaceLeftPadding;
        }
        
        // Add a border on the bottom (or on the top, depending on the view's postion)
        UIColor *borderColor = [UIColor colorWithHexString:[current valueForKey:@"borderColor"]];
        if (![TSMessage iOS7StyleEnabled])
        {
        ...
        CGFloat originX = [self iOS10StyleEnabled] ? 8.0 : 0.0;
        
        self.frame = CGRectMake(originX, topPosition, screenWidth - 2 * originX, actualHeight);
        if ([self iOS10StyleEnabled]) {
            
            self.layer.shadowColor = [UIColor blackColor].CGColor;
            self.layer.shadowOpacity = 0.4;
            CGFloat offsetY = notificationType == TSMessageNotificationTypeMessage ? 0 : 4;
...
- (CGFloat)updateHeightOfMessageView
{
    ...
    CGFloat orginY = [self iOS10StyleEnabled] && self.subtitle ? TSMessageViewiOS10StylePadding : padding;
    ...
    {
        CGFloat contentLabeOriginX = [self iOS10StyleEnabled] ? self.iconImageView.frame.origin.x + TSMessageViewiOS10StylePadding: self.textSpaceLeft;
        CGFloat contentLabeOriginY = self.titleLabel.frame.origin.y + self.titleLabel.frame.size.height + ([self iOS10StyleEnabled] ? padding + 8 : 10);
        self.contentLabel.frame = CGRectMake(contentLabeOriginX,
                                             contentLabeOriginY,
                                             screenWidth - padding - contentLabeOriginX - self.textSpaceRight,
                                             0.0);
    ...
    if (self.iconImageView)
    {
        ...
        {
            CGFloat iconCentenY = round(currentHeight / 2.0);
            if ([self iOS10StyleEnabled] && self.subtitle) {
                iconCentenY = 15;
            }
            // z-align
...
- (BOOL)iOS10StyleEnabled {
    return [TSMessage iOS10StyleEnabled];
}
